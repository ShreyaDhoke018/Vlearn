...
<p>
	...
	<?php 
		...
		$find_videos = mysqli_query($conn, "SELECT * FROM ds_videos");
		while($row = mysqli_fetch_assoc($find_videos))
		{
			$id = $row['id'];
			$name = $row['name'];
			
			echo "<a href='watch_ds.php?id=$id'>$name</a><br><br>";
		}
	?>	
	</p>
	
	</body>
	<a href="ds_vtry2.html"><button class="btn-n">Upload Files</button></a>
	</html>
